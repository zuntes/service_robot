  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  //   wheel_l_.pos = wheel_l_.calc_enc_angle(cfg_.enc_counts_per_rev);
  //   wheel_l_.vel = (wheel_l_.pos - pos_prev) / delta_seconds;

  //   pos_prev = wheel_r_.pos;
  //   wheel_r_.pos = wheel_r_.calc_enc_angle(cfg_.enc_counts_per_rev);
  //   wheel_r_.vel = (wheel_r_.pos - pos_prev) / delta_seconds;
  // }
  comms_.read_vel_values(wheel_l_.vel, wheel_r_.vel);
  ...
  RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Period %.5f:  ", period.seconds());
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  // RCLCPP_INFO(
  //      rclcpp::get_logger("DiffBotSystemHardware"), "Got command %.5f for '%s'!", wheel_l_.cmd,
  //      wheel_l_.name.c_str());
  // RCLCPP_INFO(
  //      rclcpp::get_logger("DiffBotSystemHardware"), "Got command %.5f for '%s'!", wheel_r_.cmd,
  //      wheel_r_.name.c_str());
  return hardware_interface::return_type::OK;
}
